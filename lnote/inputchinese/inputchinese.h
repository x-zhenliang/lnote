...
class InputChinese : public QWidget
{
	Q_OBJECT

public:
    explicit InputChinese(QWidget *parent = 0);
    ~InputChinese();

	//单例模式,保证一个程序只存在一个输入法实例对象
    static InputChinese *Instance()
	{
		if (!_self) {
			static QMutex mutex;
...
private:
    static InputChinese *_self;  //实例对象
